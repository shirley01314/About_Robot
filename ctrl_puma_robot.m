...
%注：DH参数和转动惯量参数是CRP公司提供，部分是自己在adams中测得的数值
%连杆长度
a1 = [-0.1573; 0.0653; 0];  %3*1矩阵
...
r1=[-0.1122; -0.1571; 0.00307];   %3*1矩阵
...
%关节角度q和关节角速度dq
q=[u(1)-pi/2;u(2);u(3);u(4)-pi/2;u(5);u(6)];  %6*1矩阵
dq=[u(7);u(8);u(9);u(10);u(11);u(12)];  %6*1矩阵
...
dq5=u(11);
dq6=u(12);
dq=[dq1;dq2;dq3;dq4;dq5;dq6];   %6*1矩阵

f=-inv(M)*N*dq;  %6*1矩阵
b=inv(M);   %6*6矩阵

%以下是控制器的设计 ---------------------------------------
x1d=[cos(pi*t);sin(pi*t/2);sin(pi*t);sin(pi*t);cos(pi*t);cos(pi*t/2)];  %期望的各关节角运动轨迹   cos弧度制
x1d0=[1;0;0;0;1;1];
dx1d=[-pi*sin(pi*t);pi/2*cos(pi*t/2);pi*cos(pi*t);pi*cos(pi*t);-pi*sin(pi*t);-pi/2*sin(pi*t/2)];   %期望的各关节的角速度轨迹
dx1d0=[0;pi/2;pi;pi;0;0];
ddx1d=[-pi*pi*cos(pi*t);-pi*pi/4*sin(pi*t/2);-pi*pi*sin(pi*t);-pi*pi*sin(pi*t);-pi*pi*cos(pi*t);-1/4*pi*pi*cos(pi*t/2)];%关节加速度轨迹
...
q0=[1;0.5;0.5;1;1;1];   %初始关节角度
dq0=[1;1;1;1;1;1];  %初始关节角速度
ddq0=[0;0;0;0;0;0]; %定义初始关节角加速度
e0=q0-x1d0;   
de0=dq0-dx1d0;
dde0=ddq0-ddx1d0;
hei1=[2 2 3 4 2 1];
...
if (t<1.5)
    p=[p1;p2;p3;p4;p5;p6];          %6*1矩阵
    dp=[dp1;dp2;dp3;dp4;dp5;dp6];   %6*1矩阵
    P=[p;dp];    %12*1矩阵
    Wt=C*P;      %6*1矩阵
    lin=C*E-Wt;  %6*1矩阵
    ddp=[ddp1;ddp2;ddp3;ddp4;ddp5;ddp6];   %6*1矩阵
    K=1;
    C2lin=C2'*lin;  %6*1矩阵
    ...
    p=[0;0;0;0;0;0];          %6*1矩阵
    dp=[0;0;0;0;0;0];   %6*1矩阵
    P=[p;dp];    %12*1矩阵
    Wt=C*P;      %6*1矩阵
    lin=C*E-Wt;  %6*1矩阵
    ddp=[0;0;0;0;0;0];   %6*1矩阵
    K=10;
    C2lin=C2'*lin;  %6*1矩阵
